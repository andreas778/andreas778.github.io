...
class ProfileView {
  ...
  setProfile(currentUser){
	...
	if (this.model.getUser(currentUser).sessions.length > 0) {
		row = this.table.getElementsByTagName("tr")[3];
		td = row.getElementsByTagName("td")[0];
		td.innerHTML = this.calculateStat();  
	}
  }
  
  
  updateNav(currentUser){
	const nick = this.model.getUser(currentUser).nickname;
...
class TimerController {
  constructor() {
	...
	this.btn = document.getElementById('saveBtn');
	if (this.btn) {
...
class Controller {
  constructor() {
	...
	this.signupView = new SignupView(this.model);
